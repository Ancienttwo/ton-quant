...
export function resolveQuantCli(): string[] {
  ...
  // Try monorepo layout first, then legacy layout
  const monorepoBackend = join(process.cwd(), "apps", "quant-backend", "src", "cli.ts");
  if (existsSync(monorepoBackend)) {
    return ["bun", "run", monorepoBackend];
  }
